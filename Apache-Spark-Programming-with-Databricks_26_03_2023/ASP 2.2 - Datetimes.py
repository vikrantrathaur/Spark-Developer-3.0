...
df = spark.read.parquet(eventsPath).select("user_id", col("event_timestamp").alias("timestamp")).withColumn("new_col", (col("timestamp")/1e6).cast('timestamp'))
display(df)
...
formattedDF = (timestampDF
               ...
               .withColumn("playingWithLowerFunc" , lit("ALLCAPS"))
              )
display(formattedDF)
...
display(formattedDF.withColumn('playingWithLowerFunc', lower(col('playingWithLowerFunc'))))
...
display(formattedDF.sort(col('user_id').desc()))
...
print(datetimeDF.groupBy(col('user_id')).agg(mean(col('second')), avg(col('second'))).first()['user_id'])
print(datetimeDF.groupBy(col('user_id')).agg(mean(col('second')), avg(col('second'))).first().user_id)
...
# display(df_test)
display(df_test.sample(fraction=0.5,seed=3))
display(df_test.sample(withReplacement=True, fraction=0.5, seed=3))
