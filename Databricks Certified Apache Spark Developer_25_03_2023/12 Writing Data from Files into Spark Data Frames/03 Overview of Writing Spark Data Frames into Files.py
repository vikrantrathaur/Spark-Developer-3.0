...
# MAGIC %md
# MAGIC 
# MAGIC * Writing files using direct APIs such as `csv`, `json`, etc under `df.write` where df is of type Spark's DataFrameWriter.
# MAGIC * Writing files using `format` and `save` under `df.write`.
...
# MAGIC * Writing into compressed files
